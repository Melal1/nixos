...
in
{
  options.my.home.waybar.theme = lib.mkOption {
    type = lib.types.enum [ "snowflake-v" "rusty" ];
    default = "rusty";
  ...
  config = lib.mkIf (windowManager == "hyprland") {
    home.file.".config/waybar/config.jsonc".source =
      config.lib.file.mkOutOfStoreSymlink (
        if theme == "snowflake-v"
        then "${waybarDir}/config-station-snowflake-v"
        else "${waybarDir}/config-station-rusty"
      );

    home.file.".config/waybar/style.css".source =
      config.lib.file.mkOutOfStoreSymlink (
        if theme == "snowflake-v"
        then "${waybarDir}/style-station-snowflake-v.css"
