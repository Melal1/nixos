...
- `pkgs/`: Custom packages (`dwm`, `dwmblocks-async`, `xwinwrap`, `easydotnet`), exposed via the overlay and as flake outputs.
...
5. Applies the current wallpaper link via `awww` if `awww-daemon` is running.

## Themes & wallpapers

Terminal themes (kitty `~/.config/kitty/current.conf`, ghostty `~/.config/ghostty/theme`) and wallpapers (`~/.local/state/theme/wallpaper` / `video`) are **mutable symlinks**: Nix manages the configs that reference these stable paths, while `theme-switch <name>` re-points the links — no rebuild needed.
...
After re-pointing, `theme-switch` re-applies the wallpaper via `awww`, restarts the mpvpaper services if the video changed, and sends reload signals to kitty (SIGUSR1) and ghostty (SIGUSR2).

Video wallpapers via mpvpaper (snowflake) are off by default; enable with `my.home.videoWallpaper.enable = true;` in `hosts/<name>/home.nix` once `~/Videos/Wallpapers` is populated. When disabled, niri falls back to `awww` with the static wallpaper link.
